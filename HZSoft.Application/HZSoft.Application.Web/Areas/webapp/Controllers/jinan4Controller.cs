...
{
    /// <summary>
    /// 广州头条：先跳转到www.jnlxsm.net:8074，
    /// 再跳转到 shop.jnlxsm.net
    /// </summary>
    public class jinan4Controller : Controller
    {
            ...
            if (!string.IsNullOrEmpty(organizeId))
            {
                ...
                if (!string.IsNullOrEmpty(organize.OrganizeId))
                {
                    ...
                    foreach (var item in entityList)
                    {
                        ...
                        styleStr +=
                        $" <li> " +
                        //$"    <a href='https://shop.jnlxsm.net/webapp/jinan4/product/{item.TelphoneID}?host={host}'>" +//跳转到135服务器详情页面
            ...
            try
            {
                ...
                if (payType == "支付宝")
                {
                    try
                    {
                        ...
                        model.QuitUrl = "https://www.jnlxsm.net:8074/webapp/jinan4/index";// 支付中途退出返回商户网站地址
                       ...
                       TenPayV3Type.NATIVE,
                        ...
                        if (result.return_code == "SUCCESS")
                        {
                            H5PayData h5PayData = new H5PayData()
                            {
                                ...
                                wx_query_href = "https://shop.jnlxsm.net/webapp/jinan4/queryWx/" + ordersEntity.Id,
                                wx_query_over = "https://shop.jnlxsm.net/webapp/jinan4/paymentFinish/" + ordersEntity.Id
                            };
...
Request.UserHostAddress, tenPayV3Info.TenPayV3Notify, TenPayV3Type.JSAPI, openId, tenPayV3Info.Key, nonceStr);
            ...
            if (result.return_code == "SUCCESS")
            {
                WFTWxModel jsApiPayData = new WFTWxModel()
                {
                    ...
                    callback_url = "https://shop.jnlxsm.net/webapp/jinan4/paymentFinish/" + ordersEntity.Id
                };
...
Request.UserHostAddress, tenPayV3Info.TenPayV3Notify, TenPayV3Type.JSAPI, openId, tenPayV3Info.Key, nonceStr);
            ...
            if (result.return_code == "SUCCESS")
            {
                WFTWxModel jsApiPayData = new WFTWxModel()
                {
                    ...
                    package = package,
                    paySign = TenPayV3.GetJsPaySign(WeixinConfig.AppID2, timeStamp, nonceStr, package, WeixinConfig.Key),
                    callback_url = "https://shop.jnlxsm.net/webapp/jinan4/paymentFinish/" + id
                };
                ViewBag.WxModel = jsApiPayData;
                LogHelper.AddLog(JsonConvert.SerializeObject(jsApiPayData));//记录日志
            }
